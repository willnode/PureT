...
class Trainer(object):
    ...
    def setup_network(self):
        ...
        model = models.create(cfg.MODEL.TYPE, \
                              vp_en=self.args.vp_en, \
                              vp_de=self.args.vp_de)
    ...
    def train(self):
        ...
        for epoch in range(self.load_epoch + 1, cfg.SOLVER.MAX_EPOCH):
            ...
            for _, (indices, input_seq, target_seq, gv_feat, att_feats, att_mask) in enumerate(self.training_loader):
                ...
                # 3、梯度清零
                self.optim.zero_grad()
                # 4（XE）、优化器lr更新（用于XE训练），在SCST时不起作用
...
def parse_args():
    ...
    parser.add_argument("--vp_en", type=int, default=0)
    parser.add_argument("--vp_de", type=int, default=0)
