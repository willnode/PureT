...
# from models.vp_att import VP_Refine_Module, VP_Attention_Module
...
class AttBasicModel(BasicModel):
    ...
    def init_hidden(self, batch_size):
        ...
        return [torch.zeros(self.num_layers, batch_size, cfg.MODEL.RNN_SIZE, device='cuda'),
                torch.zeros(self.num_layers, batch_size, cfg.MODEL.RNN_SIZE, device='cuda')]
    ...
    def preprocess(self, **kwargs):
        ...
        # embed att_feats，将2048D投影到语言模型的维度（如1024D）
        ...
        # bilinear
        # 对图像特征进行增强，并预计算语言模型部分注意力需要的Key和Value
        if cfg.MODEL.BILINEAR.DIM > 0:
    ...
    def forward(self, **kwargs): 
        ...
        outputs = torch.zeros((batch_size, seq.size(1), self.vocab_size), 
                              device='cuda')
        for t in range(seq.size(1)):
            # scheduled sampling
            if self.training and t >=1 and self.ss_prob > 0:
                prob = att_feats.new_zeros(batch_size).uniform_(0, 1)
            ...
            output = self.logit(output)  # [B, Vocab_Size]
            # Curya:
            # 此处outputs保存的不应该直接是Linear层输出，应该添加log_softmax()，才可以在SS时复原prob
            # 且直接使用Linear层输出，有一定机率导致MultiNomial出现inf问题，从而报错
            # e.g. "/....../MultinomialKernel.cu:238: sampleMultinomialOnce: block: [16,0,0], thread: [157,0,0] Assertion `!THCNumerics<scalar_t>::isinf(val)` failed."
    ...
    def decode(self, **kwargs):
        ...
        """
        sents = torch.zeros((batch_size, cfg.MODEL.SEQ_LEN), dtype=torch.long, device='cuda')
        logprobs = torch.zeros(batch_size, cfg.MODEL.SEQ_LEN, device='cuda')
        wt = torch.zeros(batch_size, dtype=torch.long, device='cuda')
        unfinished = wt.eq(wt)
        for t in range(cfg.MODEL.SEQ_LEN):
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state)
